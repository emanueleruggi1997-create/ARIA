...
const SEGMENTS = [
  { id: 'tutti', label: 'Tutti i contatti attivi', desc: 'Intera lista email', icon: '👥' },
  { id: 'instagram', label: 'Lead Instagram', desc: 'Solo contatti raccolti da IG DM', icon: '📸' },
...
export default function StepAudience({ form, setField, validContacts, allContacts, getAudienceCount, onBack, onNext }) {
  ...
  const noEmail = allContacts.filter(c => !c.email?.includes('@')).length;
  const audienceCount = getAudienceCount();

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <div style={{ fontWeight: 900, fontSize: 17, color: C.text, marginBottom: 4 }}>Audience</div>
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'Senza email', val: noEmail, color: C.warning, note: 'esclusi' },
        ].map((s, i) => (
          <div key={i} style={{ flex: '1 1 80px', textAlign: 'center', background: C.surface, borderRadius: 10, padding: '10px 8px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: C.text }}>Chi riceve questa campagna?</div>
        {SEGMENTS.map(seg => {
          ...
          return (
            <div key={seg.id} onClick={() => setField('destinatari_tag', seg.id)} style={{
      ...
      {/* Count box */}
      <div style={{
        background: C.accent2 + '11', border: `2px solid ${C.accent2}44`,
        borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16,
      }}>
        <div style={{ fontSize: 32 }}>📬</div>
        <div>
          <div style={{ fontSize: 13, color: C.muted, fontWeight: 700 }}>Questa campagna raggiungerà</div>
          <div style={{ fontSize: 32, fontWeight: 900, color: C.accent2, letterSpacing: -1 }}>
            {audienceCount} <span style={{ fontSize: 15, color: C.muted, fontWeight: 400 }}>persone</span>
          </div>
        </div>
      </div>
      ...
      {audienceCount === 0 && (
        <div style={{ background: C.warning + '15', border: `1px solid ${C.warning}44`, borderRadius: 10, padding: 12, fontSize: 13, color: C.warning }}>
          ⚠️ Nessun destinatario in questo segmento. Vai alla Mailing List e aggiungi contatti con email.
        </div>
      )}
